...
class ModelEffects2(ModelMeans):
    """
    ...
    def fit(self, data=None, group=None, k=None, cov=None, obj='REML',
        ...
        fun, grad = self.get_objective('REML')
        method = solver
        solver = Solver(method, fun, grad, self.param_vals,
        ...
        sigma, (self.mx_m, c) = self.calc_sigma()
        self.mx_r_inv = chol_inv(self.calc_r(sigma))
        self.mx_w_inv = self.calc_w_inv(sigma)[0]
        fun, grad = self.get_objective('GLS')
        solver = Solver(method, fun, grad, self.param_vals,
                        constrs=self.constraints,
                        bounds=self.get_bounds())
        res_mean = solver.solve()
        res_mean.name_obj = obj
        self.param_vals = res_mean.x
        self.update_matrices(res_mean.x)
        self.last_result = res_mean
        return res, res_mean
    ...
    def load_data(self, data: pd.DataFrame, group: str, k=None,
        ...
        self.mx_data_transformed = (self.mx_st @ self.mx_data).T @ q
    ...
    def obj_reml(self, x: np.ndarray):
        """
        REML of matrix-variate normal distribution.
        ...
        try:
            ...
            return np.inf
        data = self.mx_data_transformed
        tr_r = np.trace(r)
        m, n = self.mx_data_transformed.shape
        r_center = r_inv @ data
        center_w = data @ w_inv
    ...
    def grad_reml(self, x: np.ndarray):
        ...
        m, n = self.mx_data_transformed.shape
        for i, (d_r, d_w) in enumerate(zip(r_grad, w_grad)):
            g = 0.0
            tr_long = 0.0
            if len(d_r.shape):
    ...
    def obj_gls(self, x: np.ndarray):
        """
        GLS objective for fitting mean component given known R.
        ...
        mx = self.mx_data
        center = mx - mean.T
        return np.einsum('ij,ji->', center.T @ center, r_inv)
